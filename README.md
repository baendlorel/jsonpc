...
This is not a general-purpose JSON-with-comments parser. Comments are only valid in specific positions in order to keep it light and simple. This constraint makes the format parseable without a custom lexer: comments are converted into synthetic `_comments` properties, and the result is parsed by the native `JSON.parse`.

## Install

```bash
pnpm add json-property-comment
```
